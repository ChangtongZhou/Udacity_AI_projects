...
class AirCargoProblem(Problem):
    ...
    def get_actions(self):
        ...
        def load_actions():
            ...
            # For each cargo in each airplane in each airport, we will do load actions
            for a in self.airports:
                for p in self.planes:
                    for c in self.cargos:
                        # At(c, a) ∧ At(p, a), ^ means and
                        precond_pos = [expr("At({}, {})".format(c, a)),
                        expr("At({}, {})".format(p, a))
                        ]
                        precond_neg = [] # ¬ means not
                        effect_add = [expr("In({}, {})".format(c, p))]  # In(c, p))
                        effect_rem = [expr("At({}, {})".format(c, a))]  # not At(c, a)
        ...
        def unload_actions():
            ...
            # For each cargo in each airplane in each airport, we will do unload actions
            for a in self.airports:
                for p in self.planes:
                    for c in self.cargos:
                        # In(c, p) ∧ At(p, a)
                        ...
                        precond_neg = [] # ¬ means not
                        effect_add = [expr("At({}, {})".format(c, a))] # At(c, a)
                        effect_rem = [expr("In({}, {})".format(c, p))] # not In(c, p))
    ...
    def actions(self, state: str) -> list:
        ...
        # Put the current state's positive sentences' clauses to the propositonal logic
        kb.tell(decode_state(state, self.state_map).pos_sentence())

        for action in self.actions_list:
            possible = True
            # If the clause is positive, then check if the action is possible
            for c in action.precond_pos:
                if c not in kb.clauses:
                    possible = False
                    break

            # If the clause is negative, then the action is not executable
            for c in action.precond_neg:
                if c in kb.clauses:
                    possible = False
                    break

            # If the action is executable, add it into possible action list
            if possible:
