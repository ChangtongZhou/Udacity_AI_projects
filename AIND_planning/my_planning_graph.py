...
# Referred from https://github.com/diegocasmo/udacity-aind/tree/master/planning
# Referred from book: Artificial Intelligence: A Modern Approach
...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # initialize the level of actions
        # self.a_levels.append(set())
        for action in actions:
            nodeA = PgNode_a(action)
            # The nodeA's precondition has to be satisfied by previous state level
            if nodeA.prenodes.issubset(prev_s_level):
                # add this possible action node to the list of action nodes
                # the previous level's state nodes should be this action node's parents
                a_nodes.append(nodeA)
                for nodeS in prev_s_level:
    ...
    def add_literal_level(self, level):
        ...
        # Add the reachable effect nodes to the state level
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # If node_a1's effect is not affected by node_a2's effect return true
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.effect_rem:
                return True

        # If node_a2's effect is not affected by node_a1's effect return true
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # If node_a1's effects are not affected by node_a2's preconditions, return true
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.precond_neg:
                return True

        # If node_a2's effects are not affected by node_a1's preconditions, return true
        for effect in node_a2.action.effect_add:
            if effect in node_a1.action.precond_neg:
                return True

        # If node_a1's effects don't conflict with node_a2's preconditions, return true
        for effect in node_a1.action.effect_rem:
            if effect in node_a2.action.precond_pos:
                return True

        # If node_a2's effects don't conflict with node_a1's preconditions, return true
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        return same_symbol and is_negation
        # return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # If s1_preconds aren't mutually exclusive with s2_precond, return true
